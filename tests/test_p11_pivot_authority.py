...
class P11PivotAuthorityTests(unittest.TestCase):
    def test_core_lane_preserves_history_and_archived_disposition(self) -> None:
        ...
        self.assertTrue(any("P12-R0 completed provider-free" in rule for rule in lane["rules"]))
        self.assertTrue(any("P12-R1, P12-R2 and P12-P0 are stopped" in rule for rule in lane["rules"]))

    def test_p12_r0_remains_frozen_historical_evidence(self) -> None:
