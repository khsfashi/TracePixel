...
from .humanoid_pose import HumanoidPoseV1
from .humanoid_pose_validation import (
...
from .static_humanoid_request import (
    DETERMINISTIC_EVIDENCE_FACTS_V1,
    PERCEPTUAL_EVIDENCE_FACTS_V1,
    STATIC_HUMANOID_EVIDENCE_POLICY_SCHEMA_V1,
    STATIC_HUMANOID_REQUEST_SCHEMA_V1,
    StaticHumanoidEvidencePolicyRefV1,
    StaticHumanoidEvidencePolicyV1,
    StaticHumanoidRequestV1,
)
...
class StaticHumanoidRequestValidationError(ValueError):
    """Deterministic H3 request-binding rejection with stable code/path."""
...
def _fail(code: str, path: str, message: str) -> None:
    raise StaticHumanoidRequestValidationError(code, path, message)
...
def validate_static_humanoid_evidence_policy(value: object) -> StaticHumanoidEvidencePolicyV1:
    root = _object(value, "$policy", _POLICY_FIELDS)
    if root["schema"] != STATIC_HUMANOID_EVIDENCE_POLICY_SCHEMA_V1:
        _fail(
            "unsupported_policy_schema",
            "$policy.schema",
            f"expected {STATIC_HUMANOID_EVIDENCE_POLICY_SCHEMA_V1!r}",
        )
    ...
    if type(deterministic) is not list or tuple(deterministic) != DETERMINISTIC_EVIDENCE_FACTS_V1:
        _fail(
            ...
            "must exactly match the frozen G8-H0 deterministic evidence authority",
        )
    ...
    if type(perceptual) is not list or tuple(perceptual) != PERCEPTUAL_EVIDENCE_FACTS_V1:
        _fail(
            ...
            "must exactly match the frozen G8-H0 perceptual evidence authority",
        )
    ...
    return cast(StaticHumanoidEvidencePolicyV1, value)


def static_humanoid_evidence_policy_sha256(value: object) -> str:
    validate_static_humanoid_evidence_policy(value)
    return _canonical_sha256(value)


def validate_static_humanoid_evidence_policy_reference(
    ref: object,
    policy: object,
) -> StaticHumanoidEvidencePolicyRefV1:
    root = _object(ref, "$policy_ref", _POLICY_REF_FIELDS)
    if root["policy_schema"] != STATIC_HUMANOID_EVIDENCE_POLICY_SCHEMA_V1:
        _fail(
            "invalid_policy_schema",
            "$policy_ref.policy_schema",
            f"must equal {STATIC_HUMANOID_EVIDENCE_POLICY_SCHEMA_V1!r}",
        )
    digest = _digest(root["sha256"], "$policy_ref.sha256")
    expected = static_humanoid_evidence_policy_sha256(policy)
    if digest != expected:
        _fail(
    ...
    return cast(StaticHumanoidEvidencePolicyRefV1, ref)
...
def _validate_asset_request_context(asset_request: object) -> AssetRequestV1:
    ...
    if art_intent.get("asset_class") != "static-humanoid-character":
        _fail(
            "invalid_asset_class",
            "$asset_request.art_intent.asset_class",
            "static humanoid binding requires asset_class 'static-humanoid-character'",
        )
    return request


def validate_static_humanoid_request(
    value: object,
    *,
    asset_request: object,
    humanoid_profile: object,
    humanoid_pose: object,
    evidence_policy: object,
) -> StaticHumanoidRequestV1:
    """Validate H3 bindings without provider calls, raster generation, IK, physics, or perceptual scoring."""

    root = _object(value, "$", _REQUEST_FIELDS)
    if root["schema"] != STATIC_HUMANOID_REQUEST_SCHEMA_V1:
        _fail("unsupported_schema", "$.schema", f"expected {STATIC_HUMANOID_REQUEST_SCHEMA_V1!r}")
    ...
    profile_ref = cast(dict[str, object], root["profile_ref"])
    expected_profile_ref = {
        "kind": "morphology",
        "profile_id": profile_ref["profile_id"],
        "profile_schema": profile_ref["profile_schema"],
        "sha256": profile_ref["sha256"],
    }
    profile_refs = cast(list[object], request["profile_refs"])
    if expected_profile_ref not in profile_refs:
        _fail(
            "missing_bound_humanoid_profile",
            "$asset_request.profile_refs",
            "single-asset request must contain the exact digest-pinned humanoid profile via the existing morphology profile seam",
        )

    try:
        pose = validate_humanoid_pose(humanoid_pose, profile)
        validate_humanoid_pose_reference(root["pose_ref"], pose, profile)
    except HumanoidPoseValidationError as exc:
        _fail("pose_binding_mismatch", "$.pose_ref", str(exc))

    pose_profile_ref = cast(dict[str, object], cast(HumanoidPoseV1, pose)["profile_ref"])
    if pose_profile_ref != root["profile_ref"]:
        _fail(
            "pose_profile_binding_mismatch",
            "$.pose_ref",
            "pose and request must bind the same exact humanoid profile reference",
        )

    orientation = cast(dict[str, object], cast(HumanoidPoseV1, pose)["orientation_intent"])
    pose_facing = cast(str, orientation["facing"])
    art_intent = cast(dict[str, object], request["art_intent"])
    composition = cast(dict[str, object], art_intent["composition"])
    if composition.get("facing") != _BROAD_FACING[pose_facing]:
        _fail(
            "orientation_binding_mismatch",
            "$asset_request.art_intent.composition.facing",
            "broad ArtIntent facing must agree with the retained humanoid pose orientation",
        )

    validate_static_humanoid_evidence_policy_reference(root["evidence_policy_ref"], evidence_policy)
    return cast(StaticHumanoidRequestV1, value)
