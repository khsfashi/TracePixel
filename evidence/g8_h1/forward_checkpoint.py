...
from evidence.g8_h1 import checkpoint as legacy
...
def main() -> int:
    ...
    if type(live) is not dict:
        raise SystemExit("G8-H1 forward checkpoint failed: live core lane malformed")
    children = cast(dict[str, object], live).get("child_sequences")
    if type(children) is not dict:
        raise SystemExit("G8-H1 forward checkpoint failed: child sequence missing")
    g8 = cast(dict[str, object], children).get("G8")
    if type(g8) is not list or "G8-H1" not in cast(list[object], g8):
        raise SystemExit("G8-H1 forward checkpoint failed: G8-H1 history missing")

    historical = dict(cast(dict[str, object], live))
    historical["current"] = "G8"
    historical["current_child"] = "G8-H1"
