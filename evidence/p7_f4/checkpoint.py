...
def main() -> int:
    ...
    if lane.get("current") != "P7":
        raise SystemExit("P7-F4 checkpoint requires P7 as current core phase")
    if lane.get("current_child") not in {"P7-F4", "P7-F5"}:
        raise SystemExit("P7-F4 checkpoint only supports the F4 implementation/handoff boundary")
    if lane.get("active_issue") != 71:
        raise SystemExit("P7-F4 checkpoint requires active issue #71")
