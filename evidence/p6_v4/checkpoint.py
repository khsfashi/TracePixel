...
from tracepixel.preview import write_static_html_gallery
...
def _verify_gallery_payload() -> tuple[int, int]:
    ...
    with TemporaryDirectory(prefix="tracepixel-p6-v4-") as temporary:
        ...
        if files != ["index.html", "manifest.json"]:
            raise AssertionError(f"P6-V4 gallery payload drifted: {files!r}")
        if (output / "index.html").read_bytes() != gallery.html:
            raise AssertionError("P6-V4 publication input differs from deterministic P6-V3 HTML")
        return len(gallery.html), (output / "manifest.json").stat().st_size
