...
from evidence.p10_c4 import checkpoint as legacy
...
def main() -> int:
    ...
    if type(live) is not dict:
        raise SystemExit("P10-C4 forward checkpoint failed: live core lane malformed")
    children = cast(dict[str, object], live).get("child_sequences")
    if type(children) is not dict:
        raise SystemExit("P10-C4 forward checkpoint failed: child sequence missing")
    p10 = cast(dict[str, object], children).get("P10")
    if type(p10) is not list or "P10-C4" not in cast(list[object], p10):
        raise SystemExit("P10-C4 forward checkpoint failed: P10-C4 history missing")

    historical = dict(cast(dict[str, object], live))
    historical["current"] = "P10"
    historical["current_child"] = "P10-C4"
