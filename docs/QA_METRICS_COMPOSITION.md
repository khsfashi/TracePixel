...
P6-V2 turns the existing P6-V0 preview bundle into one deterministic review surface that places exact image evidence with deterministic QA and observational Agent-complexity evidence.
...
## Mobile-safe presentation layout

The P6-V2 review SVG uses a deterministic `vertical-stack-v1` presentation layout. This is intentionally different from the earlier two-column presentation that allowed the preview caption to intrude into the metrics column when the SVG was scaled for phone review.

The reference composition now presents, in one vertical flow:

1. the P6-V2 title,
2. the exact preview PNG,
3. a three-line bounded preview caption,
4. Deterministic QA,
5. Agent complexity,
6. the authority boundary.

The reference width is bounded to a phone-friendly minimum canvas and all review text is placed in one content column with fixed side padding. Long QA/authority text is deterministically wrapped to fixed character budgets before SVG serialization. This avoids horizontal text collision without JavaScript, browser-dependent HTML wrapping, or raster modification.

For preview PNGs wider than the reference review width, the SVG canvas expands rather than resampling the embedded source image. The source preview therefore remains at its natural dimensions and `source_image_scaling` stays `none`.
...
- the presentation layout identifier, content width and fixed text-wrap budgets,
...
- the reference composition remains at or below 400 SVG units wide,
- text is vertically ordered in one column and bounded by deterministic wrap budgets,
...
The checkpoint rebuilds the P6-V0 bundle without a provider call, creates the composition twice, requires byte-identical output, verifies the exact preview digest, deterministic QA pass state, frozen Agent-complexity values, `vertical-stack-v1` mobile layout bounds and authority separation, then materializes the files in a temporary directory.
