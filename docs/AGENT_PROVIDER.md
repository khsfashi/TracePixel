...
P5-A0 introduces the replaceable provider boundary. P5-A1 now narrows each request to one versioned compact observation without selecting a provider, model, SDK, retry policy, network transport, or cost envelope.
...
No A0/A1 function executes a PixelProgram, mutates a `Canvas`, runs raster analyzers, selects a model, performs network I/O, or turns provider output into authority.
