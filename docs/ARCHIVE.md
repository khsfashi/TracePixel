...
TracePixel began from the hypothesis that an LLM/Agent might produce stronger game-ready pixel assets when raster creation was constrained through deterministic tools, explicit stages, exact replay, deterministic QA and targeted repair rather than simpler RAW authoring or opaque image generation.

The research deliberately separated:
...
That separation remains a useful result even though the standalone product thesis was not promoted.
...
PR #157 completed the final provider-free / feature-free responsibility diff. `docs/P12_R0_RESPONSIBILITY_DIFF.md` remains the frozen record.

R0 found that local rectangular/pixel mutation and ordinary raster comparison primitives are already available in mature tooling such as Aseprite scripting and ImageMagick-style workflows. It narrowed the only remaining plausible TracePixel-specific experiment to a combined Agent-facing contract for exact source identity, allowed/protected pixel authority, protected-pixel equality proof, collateral-damage measurement and replay evidence.

That was a valid testable hypothesis, but it was not itself evidence that TracePixel still deserved an independent roadmap.

## Why P12 stops here

The owner decided that continuing to search for a new standalone TracePixel purpose is no longer worthwhile.

Therefore:

- P12-R0 is retained as completed research,
- P12-R1 experiment preregistration is stopped,
- **No P12-R1/R2 provider run is authorized**,
- P12-R2 is never executed under the archived lane,
- P12-P0 is superseded by this owner ARCHIVE decision,
...
- P12-R0 responsibility-diff document,
